...
const TagUpdateModal = ({ visible, setVisible, handleUpdate, updatingTag }) => {
  const [form, setForm] = useState({ name: updatingTag.name });

  useEffect(() => {
    setForm({ name: updatingTag.name });
  }, [updatingTag]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
      <div className="bg-gray-600 rounded-lg p-6 max-w-md w-full">
        <h2 className="text-xl font-bold mb-4">Update Tag</h2>
